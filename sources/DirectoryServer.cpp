            ...
            switch (message->getOperation()) {
                ...
                case Message::OperationType::GET_PENDING_REQUESTS:
                    reply = directoryServer->Server::saveAndGetMessage(
                            ...
                            Message::MessageType::Reply, Message::OperationType::GET_PENDING_REQUESTS);
                    break;
...
const boost::shared_ptr<DirectoryServer> &DirectoryServer::getDirectoryServer() const {
    return directoryServer;
}

void DirectoryServer::setDirectoryServer(const boost::shared_ptr<DirectoryServer> &directoryServer) {
    DirectoryServer::directoryServer = directoryServer;
}

GetPendingRequestsReply DirectoryServer::getPendingRequests(const GetPendingRequests &req) {
    GetPendingRequestsReply reply = GetPendingRequestsReply();
    const std::string &username = req.getUserName();
    const std::string &token = req.getToken();
    if (this->userExists(username)) {
        if (this->authorize(username, token)) {
            reply.setRequests(this->users[username].getUserPendingRequests());
            reply.setFlag(false);
        } else {
            reply.setFlag(true);
            reply.setMsg(ERROR_AUTH);
...
AddViewerReply DirectoryServer::acceptRequest(const AddViewerRequest &req) {
    AddViewerReply reply = AddViewerReply();
    const std::string &username = req.getUserName();
    const std::string &token = req.getToken();
    if (this->userExists(username)) {
        if (this->authorize(username, token)) {
            int i = 0;
            for (const auto &request: this->users[username].getUserRequests()) {
                i++;
                if (req.getViewerName() == request.getUserName() && req.getImageName() == request.getImageName() &&
                    req.getViewNum() == request.getViewNum()) {
                    this->users[request.getUserName()].addPendingRequest(request, true);
                    break;
                }
            }
...
DenyViewerReply DirectoryServer::denyRequest(const DenyViewerRequest &req) {
    DenyViewerReply reply = DenyViewerReply();
    const std::string &username = req.getUserName();
    const std::string &token = req.getToken();
    if (this->userExists(username)) {
        if (this->authorize(username, token)) {
            ...
            for (const auto &request: this->users[username].getUserRequests()) {
                ...
                if (req.getViewerName() == request.getUserName() && req.getImageName() == request.getImageName() &&
                    req.getViewNum() == request.getViewNum()) {
                    this->users[request.getUserName()].addPendingRequest(request, false);
                    break;
                }
            }
            this->users[username].removeRequest(i-1);
...
void DirectoryServer::User::addPendingRequest(MessageStructures::User::ViewImageRequest request, bool accepted) {
    this->pendingRequests.push_back(std::pair(request, accepted));
}

const std::vector<std::pair<MessageStructures::User::ViewImageRequest, bool>> &
DirectoryServer::User::getUserPendingRequests() const {
    return pendingRequests;
}

const std::vector<MessageStructures::User::ViewImageRequest> &DirectoryServer::User::getUserRequests() const {
    return requests;
}

void DirectoryServer::User::removeRequest(int index) {
    this->requests.erase(this->requests.begin() + index);
}

void DirectoryServer::User::removePendingRequest(int index) {
    this->pendingRequests.erase(this->pendingRequests.begin() + index);
